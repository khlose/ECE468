...
public class MicroWalker extends MicroBaseListener {
	...
	@Override public void exitCall_expr(MicroParser.Call_exprContext ctx) {
		...
		String toreg = pickle.peek().trim();
		if(parameter_replace.containsKey(toreg)){
			toreg = parameter_replace.get(toreg);
		}
		String anirudh_sivakumar = typemap.get(toreg).trim();
		//System.out.println(anirudh_sivakumar + " " + pickle.peek() +" " +  Integer.toString(count));
		for (int i = 0; i < count; i++){
			lumberjack.push(pickle.pop());
			//System.out.println(lumberjack.peek());
		}
		...
		pickle.push(newTreg);
		//Need update the type
		typemap.put(newTreg, anirudh_sivakumar);
	}
